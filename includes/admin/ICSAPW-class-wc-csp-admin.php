...
class WC_CSP_Admin {
	...
	private function includes() {
		...
		if ( WC_CSP_Core_Compatibility::is_wc_version_gte( '3.1' ) ) {
			require_once( ICSAPW_ABSPATH . 'includes/admin/ICSAPW-class-wc-csp-product-import-export.php' );
		}
		...
		// Admin AJAX.
		require_once( ICSAPW_ABSPATH . 'includes/admin/ICSAPW-class-wc-csp-admin-ajax.php' );
	}
	...
	public static function maybe_register_selectsw() {
		...
		if ( $register ) {
			...
			wp_register_script( 'sw-admin-select-init', ICSAPW_WC_()->plugin_url() . '/assets/js/select2-init' . $suffix . '.js', array( 'jquery', 'sw-admin-select' ), self::$bundled_selectsw_version );
		}
	}
	...
	public static function maybe_load_selectsw() {
		...
		if ( self::load_selectsw() ) {
			...
			wp_register_script( 'sw-admin-select', ICSAPW_WC_()->plugin_url() . '/assets/js/select2' . $suffix . '.js', array( 'jquery' ), self::$bundled_selectsw_version );

			// Register selectSW styles.
			wp_register_style( 'sw-admin-css-select', ICSAPW_WC_()->plugin_url() . '/assets/css/select2.css', array(), self::$bundled_selectsw_version );
	...
	private static function load_selectsw() {
		...
		return strpos( $load_selectsw_from, ICSAPW_WC_()->plugin_url() ) === 0;
	}
	...
	public function admin_scripts() {
		...
		if ( $is_product_page || $is_edit_product_page ) {
			...
			wp_register_script( 'wc-restrictions-writepanel', ICSAPW_WC_()->plugin_url() . '/assets/js/meta-boxes' . $suffix . '.js', array( 'jquery', 'jquery-ui-datepicker', 'wp-util', 'wc-admin-meta-boxes', 'sw-admin-select-init' ), WC_Conditional_Shipping_Payments::VERSION );
			wp_register_style( 'wc-restrictions-css', ICSAPW_WC_()->plugin_url() . '/assets/css/meta-boxes.css', array( 'woocommerce_admin_styles', 'sw-admin-css-select' ), WC_Conditional_Shipping_Payments::VERSION );
			wp_style_add_data( 'wc-restrictions-css', 'rtl', 'replace' );

		} elseif ( $is_restrictions_settings_tab ) {

			wp_register_script( 'wc-global-restrictions-writepanel', ICSAPW_WC_()->plugin_url() . '/assets/js/meta-boxes' . $suffix . '.js', array( 'jquery', 'jquery-ui-datepicker', 'wp-util', 'sw-admin-select-init' ), WC_Conditional_Shipping_Payments::VERSION );
			wp_register_style( 'wc-restrictions-css', ICSAPW_WC_()->plugin_url() . '/assets/css/meta-boxes.css', array( 'woocommerce_admin_styles', 'sw-admin-css-select' ), WC_Conditional_Shipping_Payments::VERSION );
			wp_style_add_data( 'wc-restrictions-css', 'rtl', 'replace' );
		}

		wp_register_style( 'wc-restrictions-admin-css', ICSAPW_WC_()->plugin_url() . '/assets/css/admin.css', array(), WC_Conditional_Shipping_Payments::VERSION );
	...
	*/
	...
	public function add_restrictions_settings_page( $settings ) {
		...
		$settings[] = include( ICSAPW_ABSPATH . 'includes/admin/settings/ICSAPW-class-wc-csp-settings-restrictions.php' );
	...
	*/
	...
	public function product_data_panel() {
		...
		$restrictions         = ICSAPW_WC_()->restrictions->get_admin_product_field_restrictions();
		$applied_restrictions = ICSAPW_WC_()->restrictions->maybe_update_restriction_data( $product_restrictions_meta, 'product' );
		...
		<div id="restrictions_data" class="panel csp_product_panel woocommerce_options_panel wc-metaboxes-wrapper <?php echo WC_CSP_Core_Compatibility::get_versions_class(); ?>">
			...
			<div class="options_group">
				...
				<div class="woocommerce_restrictions wc-metaboxes ui-sortable" data-hash="<?php echo $data_hash; ?>">
					...
					if ( $applied_restrictions ) {
						foreach ( $applied_restrictions as $index => $restriction_data ) {
							...
							$restriction    = ICSAPW_WC_()->restrictions->get_restriction( $restriction_id );
				...
				<div class="pro-validation">
					<span class="dashicons dashicons-lock"></span> Multiple Restrictions is only available on our <b>PRO VERSION!</b> <span>Please <a href="https://innozilla.com/wordpress-plugins/woocommerce-conditional-shipping-and-payments/#pro" target="_blank">click here</a> to purchase the PRO version of the plugin. (One time payment)</span>
				</div>
			</div>
	...
	public function get_posted_product_restriction_data() {
		...
		if ( isset( $_POST[ 'restriction' ] ) ) {
			...
			$posted_restrictions_data = sanitize_text_field($_POST[ 'restriction' ]); // @phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	...
	*/
	...
	public static function add_privacy_policy_guide_content() {
		if ( function_exists( 'wp_add_privacy_policy_content' ) ) {
			wp_add_privacy_policy_content( 'Innozilla Conditional Shipping and Payments for WooCommerce', self::get_privacy_policy_guide_message() );
		}
	}
	...
	*/
	...
	public function get_restriction_filters() {
		...
		$restrictions = ICSAPW_WC_()->restrictions->get_restrictions();
	...
	*/
	...
	public static function dismiss_notice() {
		...
		$dismissed = WC_CSP_Admin_Notices::dismiss_notice( sanitize_text_field( $_POST[ 'notice' ] ) );
