...
class WC_CSP_Restriction extends WC_Settings_API {
	...
	public function process_global_restriction_data() {
		...
		if ( isset( $_POST[ 'restriction' ] ) ) {
			$posted_restrictions_data = sanitize_text_field( $_POST[ 'restriction' ] );
		}
	...
	public function get_product_restriction_data( $product, $context = 'view' ) {
		...
		$restrictions = ICSAPW_WC_()->restrictions->maybe_update_restriction_data( $restriction_meta, 'product' );
	...
	public function get_global_restriction_data( $context = 'view' ) {
		...
		$global_restrictions = ICSAPW_WC_()->restrictions->maybe_update_restriction_data( get_option( 'wccsp_restrictions_global_settings', false ), 'global' );
	...
	public function check_conditions_apply( $restriction_data, $args = array() ) {
		...
		foreach ( $conditions as $condition_key => $condition_data ) {
			...
			if ( ! apply_filters( 'woocommerce_csp_check_condition', ICSAPW_WC_()->conditions->check_condition( $condition_data, $args ), $condition_key, $condition_data, $args, $conditions ) ) {
	...
	public function get_conditions_resolution( $restriction_data, $args = array() ) {
		...
		foreach ( $conditions as $condition_key => $condition_data ) {
			...
			$resolution = apply_filters( 'woocommerce_csp_get_condition_resolution', ICSAPW_WC_()->conditions->get_condition_resolution( $condition_data, $args ), $condition_key, $condition_data, $args, $conditions );
