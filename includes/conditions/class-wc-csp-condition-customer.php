	...
	public function check_condition( $data, $args ) {
		...
		if ( ! empty( $args[ 'order' ] ) ) {
			...
			if ( ! empty( $_POST[ 'billing_email' ] ) ) {
				...
				$check_emails[] = sanitize_email( $_POST[ 'billing_email' ] );

			// Updating order review?
			} elseif ( did_action( 'woocommerce_checkout_update_order_review' ) && ! empty( $_POST[ 'post_data' ] ) ) {
				parse_str( wp_unslash( sanitize_text_field($_POST[ 'post_data' ]) ), $billing_data ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
